...
export async function POST(request: NextRequest) {
  try {
    ...
    if (customer.balance < totalAmount) {
      return NextResponse.json({ 
        error: `Insufficient balance. Transfer amount: $${transferAmount.toFixed(2)}, Fee: $${transferFee.toFixed(2)}, Total required: $${totalAmount.toFixed(2)}, Available: $${customer.balance.toFixed(2)}` 
      }, { status: 400 });
    }
